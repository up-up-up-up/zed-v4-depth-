    ...
    while (exit_flag == True):
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            for cl, score, (left, top, width, height) in zip(classes, confidences, boxes):
                ...
                cv2.putText(img, "x: " + str(round(point_cloud_value[0] / 1000, 2)) + 'm',
                            (int(left), int(top-100)),
                            cv2.FONT_HERSHEY_COMPLEX, 1, COLORS[2], 2)
                cv2.putText(img, "y: " + str(round(point_cloud_value[1] / 1000, 2)) + 'm',
                            (int(left), int(top-75)),
                            cv2.FONT_HERSHEY_COMPLEX, 1, COLORS[2], 2)
                cv2.putText(img, "z: " + str(round(point_cloud_value[2] / 1000, 2)) + 'm',
                            (int(left), int(top-50)),
                            cv2.FONT_HERSHEY_COMPLEX, 1, COLORS[2], 2)
